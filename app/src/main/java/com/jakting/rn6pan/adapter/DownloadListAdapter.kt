    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        holder.itemView.setOnClickListener {
            when(downloadListModify[position].state){
                2->Aria.download(this).load(downloadListModify[position].id).ignoreCheckPermissions().resume()//暂停中，要进行
                4->Aria.download(this).load(downloadListModify[position].id).ignoreCheckPermissions().stop()//进行中，要暂停
            }
        }
            ...
            popup.setOnMenuItemClickListener {
                ...
                when (it.itemId) {
                    R.id.menu_transmission_download_delete -> {
                        Aria.download(this).load(downloadListModify[position].id).ignoreCheckPermissions().cancel(true)
                    }
